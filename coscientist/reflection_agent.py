...
# NOTE: gpt_researcher is imported lazily inside the web-research functions so
# that this module (desk-reject, simulation, deep verification, corpus grounding)
# can be imported and used offline without the heavy web-research dependency.
...
def assumption_decomposer_node(
    state: ReflectionState, llm: BaseChatModel, max_assumptions: int = 0
) -> ReflectionState:
    """
    ...
    max_assumptions: int, default=0
        When > 0, keep only the first ``max_assumptions`` top-level assumptions
        for research (LITE mode's top-K cap). 0 = research all.
    ...
    # LITE cap: research only the top-K assumptions (the decomposer lists the
    # primary ones first), which bounds the per-hypothesis web-search fan-out.
    if max_assumptions and len(parsed_assumptions) > max_assumptions:
        parsed_assumptions = dict(list(parsed_assumptions.items())[:max_assumptions])
...
def deep_verification_node(
    ...
    # Optionally cross-check against the lab's private corpus.
    private_corpus_grounding = ""
    if retriever is not None:
        private_corpus_grounding = retriever.ground(
            state["hypothesis_to_review"].hypothesis
        ).context_block
    elif is_tailings_related(state["hypothesis_to_review"].hypothesis):
        private_corpus_grounding = tailings_guide_block()
    ...
    prompt = load_prompt(
        ...
        private_corpus_grounding=private_corpus_grounding,
    )
    response = llm.invoke(prompt)
...
def build_deep_verification_agent(
    ...
    retriever: Optional[CorpusRetriever], default=None
        Private-corpus retriever used for corpus-only grounding.
    web_research_enabled: bool, default=True
        Whether assumption research may call GPTResearcher/web search.
    ...
    graph.add_node(
        "assumption_decomposer",
        lambda state: assumption_decomposer_node(state, llm, max_assumptions=max_assumptions),
    )

    # Choose research node based on the web gate first. With web disabled,
    # reflection must stay offline and use only private-corpus evidence.
    if not web_research_enabled:
        graph.add_node(
            "assumption_researcher",
            lambda state: _corpus_assumption_research_node(state, retriever),
        )
    elif parallel:
        graph.add_node(
            "assumption_researcher",
    ...
    graph.add_node(
        "deep_verification",
        lambda state: deep_verification_node(state, review_llm, retriever),
    )
...
def _assumption_query(
    assumption: str,
    sub_assumptions: list[str],
    *,
    latest_research: bool = False,
) -> str:
    qualifier = " using the latest research" if latest_research else ""
    query = (
        "Assess the validity of the following assumption and each "
        f"of its sub-assumptions{qualifier}. "
...
def _corpus_assumption_research_node(
    state: ReflectionState,
    retriever: Optional["CorpusRetriever"] = None,
) -> ReflectionState:
    """
    Produce assumption research without web access.

    When a private corpus is available, each assumption is grounded against it.
    Otherwise, keep a clear local note so the downstream verification prompt has
    the required state key but no web-backed evidence is implied.
    """
    parsed_assumptions = state.get("_parsed_assumptions") or {}
    if not parsed_assumptions:
        parsed_assumptions = {
            "Web research disabled": [
                "No parsed assumptions were available for offline research."
            ]
        }
    ...
    for assumption, sub_assumptions in parsed_assumptions.items():
        if retriever is None:
            if is_tailings_related(assumption, *sub_assumptions):
                assumption_research_results[assumption] = (
                    "Web research disabled; using built-in tailings-report "
                    "domain guidance for this assumption.\n\n"
                    + tailings_guide_block()
                )
            else:
                assumption_research_results[assumption] = (
                    "Web research disabled; no private corpus evidence was available "
                    "for this assumption."
                )
            continue
        context = retriever.ground(_assumption_query(assumption, sub_assumptions))
        assumption_research_results[assumption] = (
            context.context_block
            or "Web research disabled; private corpus returned no matching evidence."
        )
...
def _parallel_assumption_research_node(
    ...
    async def _conduct_research():
        ...
        research_tasks = []
        for assumption, sub_assumptions in parsed_assumptions.items():
            task = _write_assumption_research_report(
                _assumption_query(
                    assumption, sub_assumptions, latest_research=True
                )
            )
...
def _sequential_assumption_research_node(
    ...
    for assumption, sub_assumptions in parsed_assumptions.items():
        ...
        result = asyncio.run(
            _write_assumption_research_report(
                _assumption_query(
                    assumption, sub_assumptions, latest_research=True
                )
            )
