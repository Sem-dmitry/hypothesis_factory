...
class IndependentConfig:
    ...
    llm: BaseChatModel
    # Optional private-corpus retriever. When set, the generator grounds its
    # hypothesis in cited evidence from the lab's own documents.
...
class CollaborativeConfig:
    ...
    max_turns: int = 10
    # Optional private-corpus retriever shared by all collaborating agents.
    retriever: Optional["CorpusRetriever"] = None
...
def build_generation_agent(
    ...
    if mode == "independent":
        ...
        return _build_independent_generation_agent(
            config.field, config.reasoning_type, config.llm, config.retriever
        )
...
def _independent_generation_node(
    ...
    # Optionally ground the hypothesis in the lab's private corpus.
    private_corpus_grounding = ""
    if retriever is not None:
        private_corpus_grounding = retriever.ground(state["goal"]).context_block
    elif is_tailings_related(state["goal"]):
...
def _build_independent_generation_agent(
    ...
    graph.add_node(
        "generator",
        lambda state: _independent_generation_node(
            state, field, reasoning_type, llm, retriever
        ),
    )
...
def _build_collaborative_generation_agent(
    ...
    for agent_name in agent_names:
        agent_node_fns[agent_name] = multiturn.create_agent_node_fn(
            ...
            prompt_keys_from_state=["goal", "literature_review", "meta_review", "constraints", "user_guidance"],
            retriever=retriever,
