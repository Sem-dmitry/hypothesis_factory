...
from coscientist.web_literature import append_web_references_to_review
...
class CoscientistState:
    ...
    def __init__(self, goal: str, constraints: str = ""):
        self.goal = goal
        # Hard requirements the hypotheses must respect (equipment, budget, regs).
        self.constraints = constraints
        # Live steering messages the human expert sends WHILE the run executes.
        # Folded into agent prompts at supervisor-loop boundaries (chat-to-agents).
        self.user_guidance = []
        ...
        # Structured, cited per-hypothesis assessments (case output shape).
        self.assessments = []
        # Retry/skip bookkeeping surfaced to the supervisor (label -> count), so
        # the planner can react to steps that failed (like the for_inspiration
        # project surfacing failed subtask status to its planner).
...
class CoscientistStateManager:
    ...
    @property
    def literature_review_reports(self) -> list[str]:
        """Raw Literature subtopic reports, if available."""
        if self._state.literature_review is None:
            return []
        reports = self._state.literature_review.get("subtopic_reports", [])
        return [str(report) for report in reports if str(report).strip()]
    ...
    @property
    def assessments(self) -> list:
        """
        Get the stored structured hypothesis assessments (may be empty).

        Uses getattr so states pickled before this field existed still load.
        """
        return getattr(self._state, "assessments", None) or []

    @_maybe_save(n=1)
    def set_assessments(self, assessments: list) -> None:
        """Store the ranked structured hypothesis assessments."""
        self._state.assessments = list(assessments)

    @property
    def execution_issues(self) -> dict:
        """Retry/skip counts recorded during the run (backward compatible)."""
        issues = getattr(self._state, "execution_issues", None)
        if issues is None:
            self._state.execution_issues = {}
        return self._state.execution_issues

    @_maybe_save(n=1)
    def record_execution_issue(self, label: str, count: int = 1) -> None:
        """Record that a step was retried out / skipped (e.g. an empty model
        response). Surfaced to the supervisor so it can react."""
        issues = self.execution_issues
        issues[label] = issues.get(label, 0) + count

    def execution_issues_summary(self) -> str:
        """Human-readable summary of skipped/fallback steps for the supervisor."""
        issues = dict(self.execution_issues)
        # Fold in tournament match fallbacks tracked on the tournament itself.
        fallback = getattr(self._state.tournament, "fallback_count", 0) or 0
        if fallback:
            issues["Tournament match fallback (random winner)"] = fallback
        if not issues:
            return "None"
        return "; ".join(f"{label}: {count}" for label, count in issues.items())

    def top_tournament_hypotheses(self, k: int = 5) -> list[str]:
        """
        Return the hypothesis statements of the top-``k`` tournament entries,
        ordered by Elo rank. Empty when the tournament has no hypotheses.
        """
        if self._state.tournament is None:
            return []
        sorted_hypotheses = self._state.tournament.get_sorted_hypotheses()
        texts: list[str] = []
        for uid, _rating in sorted_hypotheses[: max(0, k)]:
            hypothesis = self._state.tournament.hypotheses.get(uid)
            if hypothesis is not None:
                texts.append(hypothesis.hypothesis)
        return texts
    ...
    def num_actions(self) -> int:
        """Number of supervisor actions taken so far (for the budget cap)."""
        return len(self._state.actions)

    def recent_actions(self, n: int) -> list[str]:
        """The last ``n`` supervisor actions (most recent last); for run guards."""
        return list(self._state.actions[-n:]) if n > 0 else []

    @property
    def constraints(self) -> str:
        """Constraints the hypotheses must respect (backward compatible)."""
        return getattr(self._state, "constraints", "") or ""

    @constraints.setter
    def constraints(self, value: str) -> None:
        self._state.constraints = value or ""

    @property
    def guidance_text(self) -> str:
        """Accumulated live steering from the human expert (may be empty)."""
        msgs = getattr(self._state, "user_guidance", None) or []
        return "\n".join(f"- {m}" for m in msgs)

    @property
    def num_guidance(self) -> int:
        return len(getattr(self._state, "user_guidance", None) or [])

    @_maybe_save(n=1)
    def add_guidance(self, message: str) -> None:
        """Record a live steering message; folded into agent prompts next step."""
        message = (message or "").strip()
        if not message:
            return
        if not hasattr(self._state, "user_guidance") or self._state.user_guidance is None:
            self._state.user_guidance = []
        self._state.user_guidance.append(message)
    ...
    def next_generation_state(
        ...
        literature_review_content = append_web_references_to_review(
            "\n\n".join(self._state.literature_review["subtopic_reports"])
        )
        ...
        base_state = {
            ...
            "constraints": getattr(self._state, "constraints", "") or "",
            "user_guidance": self.guidance_text,
        }
    ...
    def next_supervisor_state(
        self, max_actions: int = 40, max_hypotheses: int = 0
    ) -> SupervisorDecisionState:
        """
        Create an initial state for the supervisor agent.

        ``max_actions`` is the soft action budget (safety cap) surfaced to the
        supervisor so it can wind down as the budget is spent.
        """
        ...
        if self._state.tournament is not None:
            ...
            rel = self._state.tournament.relative_trajectory_signals()
        else:
            ...
            median_elo_rating = "[]"
            rel = {
                "has_clear_leader": False, "is_plateau": False, "leader_gap": 0.0,
                "max_elo_delta_recent": 0.0, "num_contenders": 0,
            }
        ...
        return SupervisorDecisionState(
            ...
            execution_issues=self.execution_issues_summary(),
            has_clear_leader="yes" if rel["has_clear_leader"] else "no",
            is_plateau="yes" if rel["is_plateau"] else "no",
            leader_gap=str(rel["leader_gap"]),
            max_elo_delta_recent=str(rel["max_elo_delta_recent"]),
            num_contenders=str(rel["num_contenders"]),
            actions_budget=f"{len(self._state.actions)} used / {max_actions} budget",
            hypotheses_budget=(
                f"{self.total_hypotheses} of max {max_hypotheses}"
                if max_hypotheses and max_hypotheses > 0
                else f"{self.total_hypotheses} (no cap)"
            ),
            user_guidance=self.guidance_text,
        )
