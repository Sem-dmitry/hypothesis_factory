...
from coscientist.common import RETRY_FAILED, load_prompt, retry_call
...
# --- Relative (adaptive) termination signals ------------------------------
# The finish decision is expressed relative to the run's own rating trajectory
# and distribution rather than absolute Elo numbers, so it adapts to the model
# tier (a strong hypothesis tops out lower on a light model) and to the goal.
# All thresholds are multiples of K_FACTOR (~one decisive win) so they scale
# with the tournament dynamics instead of being magic constants.
PLATEAU_WINDOW = 3          # tournaments to look back for improvement
PLATEAU_EPS = K_FACTOR      # top rating moved < ~one decisive win over the window ⇒ plateau
LEADER_GAP_K = 2            # clear leader when (max − median) ≥ LEADER_GAP_K * K_FACTOR
...
class EloTournament:
    ...
    def __init__(self, goal: str):
        ...
        # Count of matches decided by random fallback (unparseable judgments),
        # surfaced to the supervisor so it knows the Elo ratings carry noise.
    ...
    def _determine_winner(
        ...
        def _attempt() -> tuple[int, str]:
            ...
            if not (("1" in winner_str) ^ ("2" in winner_str)):
                raise ValueError(f"Invalid winner string: {winner_str}")
            winner = 1 if "1" in winner_str else 2
            return winner, response_text

        # Retry on empty/malformed judgments; if all fail, pick a winner at
        # random so a single bad debate doesn't abort the whole tournament.
        result = retry_call(_attempt, label=f"Tournament match ({prompt_name})")
        if result is RETRY_FAILED:
            self.fallback_count = getattr(self, "fallback_count", 0) + 1
            winner = random.choice([1, 2])
            return winner, (
                f"[fallback] Could not determine a winner after retries; "
                f"picked hypothesis {winner} at random to keep the tournament running."
            )
        return result
    ...
    def relative_trajectory_signals(self) -> dict:
        """
        Adaptive finish signals derived from the run's OWN rating distribution
        and trajectory — no absolute Elo thresholds. This lets the supervisor
        decide when to stop based on *how the process is going* (a strong
        hypothesis tops out lower on a light model, but still "separates" and
        "plateaus"), rather than a magic number.

        Returns keys: max_elo_delta_recent, leader_gap, rating_spread,
        is_plateau, has_clear_leader, num_contenders, rounds.
        """
        rounds = self._past_tournament_ratings
        n = len(rounds)
        signals = {
            "max_elo_delta_recent": 0.0,
            "leader_gap": 0.0,
            "rating_spread": 0.0,
            "is_plateau": False,
        ...
        latest = rounds[-1]
        latest_max = max(latest)
        latest_median = statistics.median(latest)
        latest_min = min(latest)
        signals["leader_gap"] = round(latest_max - latest_median, 1)
        signals["rating_spread"] = round(latest_max - latest_min, 1)
        signals["num_contenders"] = sum(1 for r in latest if r >= latest_max - K_FACTOR)
        # Clear leader: the top is separated from the pack by ≥ a couple of
        # decisive wins — relative to the median, so it adapts to the level.
        signals["has_clear_leader"] = signals["leader_gap"] >= LEADER_GAP_K * K_FACTOR

        # Plateau: the best rating stopped climbing over the recent window.
        if n >= 2:
            back = rounds[max(0, n - 1 - PLATEAU_WINDOW)]
            delta = latest_max - max(back)
            signals["max_elo_delta_recent"] = round(delta, 1)
            signals["is_plateau"] = delta < PLATEAU_EPS
        return signals
