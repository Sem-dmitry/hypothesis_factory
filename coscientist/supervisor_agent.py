...
class SupervisorDecisionState(TypedDict):
    ...
    # Relative (adaptive) termination signals — replace absolute Elo thresholds.
    has_clear_leader: str
    is_plateau: str
    leader_gap: str
    max_elo_delta_recent: str
    num_contenders: str
    actions_budget: str
    # Current pool size vs. the hard hypothesis cap ("T of max M" or "T (no cap)").
    hypotheses_budget: str
    # Live steering from the human expert during the run (may be empty).
    user_guidance: str
    action: str
...
def _supervisor_decision_node(
    ...
    prompt = load_prompt(
        ...
        execution_issues=state.get("execution_issues", "None"),
        hypotheses_budget=state.get("hypotheses_budget", ""),
