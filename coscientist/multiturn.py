from typing import TYPE_CHECKING, Any, Callable, Optional, Type, TypedDict
...
def create_agent_node_fn(
    ...
    **prompt_kwargs: dict[str, Any],
    ...
    When ``retriever`` is provided, a cited private-corpus grounding block for
    the run's goal is injected into the prompt as ``private_corpus_grounding``.
    """
    assert (
    ...
    def agent_fn(state):
        ...
        # Optionally ground the discussion in the lab's private corpus.
        if retriever is not None:
            prompt_kwargs["private_corpus_grounding"] = retriever.ground(
                state.get("goal", "")
            ).context_block
        elif is_tailings_related(state.get("goal", "")):
            prompt_kwargs["private_corpus_grounding"] = tailings_guide_block()
