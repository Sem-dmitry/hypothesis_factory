...
from hypotheses_page import display_hypotheses_page
...
def main():
    ...
    elif page == "Ranked Hypotheses":
        if state is None:
            st.info(
                "👈 Please select a research goal or upload a Coscientist state file from the sidebar to get started."
            )
            st.markdown("""
            ## Ranked Hypotheses Page

            The case deliverable: hypotheses ranked by the weighted score, each with its
            justification, mechanism of influence, novelty, technical/economic risks, expected
            value, target-KPI impact, verification roadmap and **cited sources** from the private
            corpus. One-click export to Markdown, HTML, CSV, JSON and Jira/YouTrack task payloads.

            Populated after a full `CoscientistFramework.run()` completes (the assessment step),
            or after calling `framework.assess_hypotheses()` on a loaded state.
            """)
        else:
            display_hypotheses_page(state)
